    ...
    public sealed class FreeListManager : IDisposable
    {
        ...
        private void Load()
        {
            ...
            try
            {
                ...
                uint count = Utils.StreamUtil.DecodeVariableByteInteger(stream); //buffer[0] << 24 | buffer[1] << 16 | buffer[2] << 8 | buffer[3];
                for (int i = 0; i < count; ++i)
                {
                    ...
                    int pos = (int)Utils.StreamUtil.DecodeVariableByteInteger(stream); //buffer[0] << 24 | buffer[1] << 16 | buffer[2] << 8 | buffer[3];
                    int length = (int)Utils.StreamUtil.DecodeVariableByteInteger(stream); //buffer[4] << 24 | buffer[5] << 16 | buffer[6] << 8 | buffer[7];
        ...
        public void Save()
        {
            ...
            Utils.StreamUtil.EncodeVariableByteInteger((uint)count, stream);
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                //buffer[2] = (byte)(pos >> 8);
                //buffer[3] = (byte)pos;
                Utils.StreamUtil.EncodeVariableByteInteger((uint)pos, stream);
                ...
                Utils.StreamUtil.EncodeVariableByteInteger((uint)length, stream);
            }
